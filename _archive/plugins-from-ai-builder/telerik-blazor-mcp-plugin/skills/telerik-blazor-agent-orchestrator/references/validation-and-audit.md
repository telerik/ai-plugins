...
Validation rules are loaded in this phase. Do not retrieve a separate `context-brief-validation.md`; there is no validation context shard. Load `../../telerik-blazor-validator/SKILL.md`, `../../telerik-blazor-orchestrator/validation-steps.md`, and validation-relevant building blocks summarized in context briefs.

## Telerik Fact Source Policy

- Tester and audit compare generated source against context briefs, explicit handoff facts, prior reports, Blazor validation references, and static guidance supplied by this pipeline.
- Tester and audit may inspect application source and project files for local usage, dependency presence, Razor/Telerik setup, and theme/static asset validation.
- Tester and audit must not inspect NuGet package internals, generated dependency source, Telerik assemblies/decompiled code, `bin`, `obj`, generated Razor artifacts, package caches, or dependency declaration files to recover missing Telerik facts.
- If a required Telerik/design-system/accessibility fact is absent from provided briefs or handoff facts, route it as `context_required` / `NEEDS_CONTEXT`; do not fill the gap from dependency internals. `telerik_validator_assistant` is a validation gate, not a documentation source.

## Rule Loading

Load validation references once before tester dispatch:

- `plugins/telerik-blazor-mcp-plugin/skills/telerik-blazor-validator/SKILL.md`
- `plugins/telerik-blazor-mcp-plugin/skills/telerik-blazor-orchestrator/validation-steps.md`

Extract Razor validator requirements, Blazor setup checks, Telerik component validation checks, and Kendo Design System utility checks.

Load audit checklist material from context brief summaries plus `telerik-blazor-orchestrator/ux-guidelines.md`, `telerik-blazor-accessibility/wcag-guidelines.md`, and component-specific accessibility facts from context briefs.

Accessibility is part of the validation and audit scope, aligned with the Angular workflow. It is not an `active_dimensions` value and does not create a standalone context-retrieval shard. WCAG baseline rules come from validation/audit references, and component-specific ARIA / keyboard / focus facts come from scoped component context briefs.

## Build `files_to_check`

Read the leading JSON status block and `Files written` section of every developer report. Build a deduplicated flat list `files_to_check`; latest write wins per path. Exclude dependency and generated folders by policy: `bin`, `obj`, generated Razor artifacts, package caches, NuGet package internals, decompiled assemblies, source maps, and generated dependency source.
...
Dispatch `telerik-blazor-tester` first. Do not dispatch `telerik-blazor-audit` for the same iteration until tester returns `GREEN`, unless tester is explicitly unavailable due to an external non-code blocker.
...
```text
Handoff to: telerik-blazor-tester
- verification_scope:
    validator_rules: <rules from validator skill and validation steps>
    component_api_facts: <parameters/events/child content/usings/setup from scoped summaries; include component-specific accessibility facts when present>
...
Tester consumes `developer_self_check` first. If `fileState = final` and a source/static/build/validator check is listed in `passed`, tester should mark that same check as already completed and continue with uncovered validation work. Tester must not mark runtime smoke, a11y smoke, browser checks, unit tests, e2e tests, or integration tests complete from developer self-check data. Rerun checks when self-check data is missing, stale, failed, skipped, incomplete, or contradicted by the current scope.

## Audit Dispatch After Green

Dispatch `telerik-blazor-audit` only after tester returns `GREEN` for the current iteration. Audit does not read tester output.

```text
Handoff to: telerik-blazor-audit
- audit_scope:
    quality_checklist: <UX guidelines + Blazor setup quality + Telerik component quality, filtered by active dimensions; WCAG baseline always included>
...
Keep static validation/audit guidance available, but apply it only to loop files and dimensions. Expand beyond the current loop task only when a finding, compiler error, Razor validator error, or runtime error names an additional affected file or dimension.
...
- `BLOCKED` (`fix_required` findings, Razor validator/build/runtime failure, or external validation blocker) -> fix loop, except external blockers that require user intervention.

For trivial UI Telerik enforcement, route `[context_required] dimension=components` findings before fix-loop dispatch when tester says the Telerik replacement fact was not previously fetched. After the context addendum lands, rerun tester or dispatch a targeted fix loop with the new component fact. If tester already has the Telerik replacement fact, route the finding directly to the fix loop.
...
Tester report caps: for green paths, rely on the JSON block and keep Markdown concise. For blocked paths, include at most 3 unique critical validator/build/runtime errors and at most 5 grouped a11y rule findings.

## Layout Utility Validation

When `layout` is active, tester must validate Kendo Design System utility availability through the current Blazor layout conventions:

- required package/static asset/CDN reference or an existing project-approved utility import is present;
- required theme/static asset reference is wired through the detected Blazor app mechanism;
- Razor `@@` escaping is correct where CSS examples require literal `@`.

Detection facts that tester cannot determine from source/project files route to context loop. Missing package, static asset, CDN reference, utility import, or theme/static asset wiring routes to fix loop.
