...
- <Telerik component or expected component category> - <purpose in the UI>
...
### Blazor setup expectations
- <TelerikRootComponent, _Imports.razor, Program.cs, project/package/theme utility expectations implied by context>
...
Do not perform project detection for developer. The developer detects Blazor app model, route/style/layout setup, project files, and implementation files. Tester may still perform targeted validation checks later from the developer report and source files.

## 1.5b - Developer Task Strategy

Prefer a single autonomous developer handoff. Parallel developer tasks are only safe when requested work is truly independent and overlapping edits to routes, `_Imports.razor`, root layouts, project files, styles, and Telerik setup are unlikely.
...
- Telerik component common guidelines: <brief path or source file summary>
...
- Validator expectations: <brief path or source file summary>

## Developer task
- task: <one concrete autonomous implementation task>
- user_goal: <original request or relevant slice>
- acceptance_criteria: <compact bullets from layout-contract.md, user goal, and Blazor static guidance requirements>
- known_context:
    ...
    - static_guidance_notes: <compact reminders for TelerikRootComponent, no attribute splatting, Kendo utilities, WCAG, validation>
