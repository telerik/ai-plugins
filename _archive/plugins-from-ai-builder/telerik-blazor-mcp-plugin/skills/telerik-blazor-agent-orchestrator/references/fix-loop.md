...
- `telerik-blazor-tester BLOCKED` with fix-required findings, Razor validator errors, build failure, runtime failure, or other code-level validation failure.
- `telerik-blazor-audit DONE_WITH_CONCERNS`.
- `telerik-blazor-audit BLOCKED` when the blocker is fixable by source changes.
- `telerik-blazor-developer DONE_WITH_CONCERNS` with fix-loop candidates or self-check concerns that can be fixed from existing context.
- `telerik-blazor-developer BLOCKED` when a narrower listed fix-loop candidate can be safely dispatched.

Tester runs before audit. Enter the fix loop immediately when tester returns fix-required findings, validator errors, build/runtime failure, or actionable validation blockers; do not wait for audit in that iteration. Audit findings enter a later fix loop only after tester is green and audit has run. Developer-originated fix-loop candidates may start a pre-validation fix loop when they are discovered before tester dispatch.

External blockers that require user intervention should be surfaced instead of entering the fix loop.

## Build Fix Task

Use findings from `validation-brief.md`, `audit-brief.md`, or developer `Batched loop requests` to build one autonomous fix task for `telerik-blazor-developer`. The orchestrator summarizes what failed and passes relevant report paths and context, but it does not preassign files to edit.
...
Map common Blazor/Telerik findings into acceptance criteria:

- Invalid Telerik parameter/event/child content -> correct using scoped component facts or request context first.
- Unsupported attribute splatting/style/class on Telerik component -> use supported component parameters or wrapper elements per common guidelines.
- Missing TelerikRootComponent, Telerik service registration, `_Imports.razor` using, theme reference, or Kendo utility setup -> inspect project and make the minimal setup edit.
...
- `loop_component_facts`: only component facts needed for Telerik components in `loop_files`.
...
```text
Handoff to: telerik-blazor-developer
...
- acceptance_criteria:
    ...
    - <relevant Blazor building-block rules remain satisfied>
- known_context:
...
2. Update the current loop task: `loop_files` becomes files modified by the fix plus shared layout, route, `_Imports.razor`, root CSS, `Program.cs`, project, or static asset files that could affect them; keep only still-relevant findings, dimensions, brief paths, and section hints.
3. Re-run tester for the current loop task when the original round had tester findings, Razor validator errors, build failures, or the fix touched usings, Telerik setup, `_Imports.razor`, routing, project/static asset config, theme setup, or Razor/template bindings.
4. Re-run audit for the current loop task when the original round had audit findings or the fix touched Razor UI, styles, accessibility, behavior, shared layout, routing, theme/static assets, or configuration.
