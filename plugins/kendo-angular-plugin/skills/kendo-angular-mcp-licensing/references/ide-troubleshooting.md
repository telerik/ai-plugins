# IDE Troubleshooting — Kendo Angular MCP Server
...
vs_code_log_location:
    ...
    - Select 'kendo-angular-mcp-server' from the dropdown.
...
handle_syntax_by_ide:
    ...
    cursor: >
        ...
        or reference the server via @kendo-angular-mcp-server. The # prefix
...
recommended_resolution:
    ...
    - In the popup, check 'kendo-angular-mcp-server' from the list to enable it.
...
```text
If the Kendo Angular MCP server assistants are not available in GitHub Copilot, please check the following:
...
3. In the popup, enable 'kendo-angular-mcp-server'.
4. Restart VS Code.
```
...
Use when a customer reports that a known-fixed behavior or specific tool is still broken after updating `@progress/kendo-angular-mcp`.

```yaml
strong_signals:
    - Customer says they updated kendo-react-mcp but behavior is unchanged.
    - 'npx -y @progress/kendo-angular-mcp@latest' is silently using a cached older version.

recommended_resolution:
    - Force a fresh download: npx --yes --ignore-existing @progress/kendo-angular-mcp@latest
...
avoid:
    - Assuming 'npx -y @progress/kendo-angular-mcp@latest' always fetches the true latest version.
...
strong_signals:
    ...
    - 'npx -y @progress/kendo-angular-mcp@latest' hangs or fails on the corporate network but works on a personal machine or VPN bypass.
...
recommended_resolution:
    ...
    - Consider pre-installing: 'npm install -g @progress/kendo-angular-mcp' so npx uses the globally installed version without a network call.
