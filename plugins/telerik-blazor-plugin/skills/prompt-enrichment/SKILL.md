...
|---|---|---|---|
| The prompt is vague or generic (under ~30 words, no layout/data/component specifics, generic terms like "dashboard", "page", "app"). The user wants a UI but hasn't said what's in it. | A detailed design brief: user role, information architecture, spatial layout with dimensions, Telerik component plan, realistic sample data, density/tone, and interaction spec. | Single-view enrichment | `references/enrichment-dimensions.md` |
...
| You need to select a spatial layout pattern for any view (during enrichment or scaffolding). | A specific layout pattern with spatial structure, Telerik components, and "when NOT to use" guidance. | Layout selection | `references/layout-patterns.md` |
...
5. **Don't duplicate implementation.** Defer to Telerik Agentic UI Generator assistants (`#telerik_component_assistant`, `#telerik_layout_assistant`, etc.) for API usage, code, and theming. This skill produces the design brief they consume.
...
### Component Plan
1. [Name] — [Telerik component] — [What it shows, with sample values]
2. ...
