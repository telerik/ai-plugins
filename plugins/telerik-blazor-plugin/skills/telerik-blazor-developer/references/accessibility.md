# Telerik UI for Blazor — Accessibility Guidelines (WCAG 2.2 Level AA)
...
- ✅ CORRECT: Logical hierarchy with descriptive headings
- ❌ WRONG: Skipped levels (h1→h3), non-descriptive headings

## 8. Page Component Landmark Rules (WCAG 1.3.1, 4.1.2)
Page components may be rendered inside a layout that already provides `<main>`, `<header>`, or `<footer>`. Since the layout configuration is unknown, avoid using these landmarks in page components to prevent potential duplication and nesting violations. Use `<div>`, `<section>`, or `<article>` as the top-level wrapper instead.
...
- **Structure**: One h1, logical heading hierarchy, avoid `<main>`, `<header>`, or `<footer>` in page components (the layout may already provide them), use `<section>` or `<div>` as page-level wrappers, semantic HTML
