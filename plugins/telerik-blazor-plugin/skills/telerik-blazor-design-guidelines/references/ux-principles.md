...
Telerik Blazor uses a `Size` parameter across most components. Use it consistently within a view:

| Tier | `Size` value | Base unit | Best for |
|---|---|---|---|
| Compact | `"sm"` | 24px row height | Data-dense grids, control panels |
| Default | `"md"` (omit) | 32px row height | Standard enterprise UI |
| Comfortable | `"lg"` | 40px row height | Forms, onboarding, public-facing |
...
Telerik Blazor applications are almost always fixed-width layouts inside a shell.
...
Do not use TelerikSplitter as a page layout tool. Use CSS Grid for page structure.
...
- **No looping animations** except loading spinners (which Telerik Blazor handles via its own loading overlay).
...
| Pattern | Use when |
|---|---|
| TelerikGrid loading overlay (`Loading`) | Grid data is refreshing |
| TelerikLoader / TelerikProgressBar (indeterminate) | Long background process without progress info |
| Skeleton loader (custom) | Initial page load of structured content |
| TelerikButton `Enabled="false"` + spinner icon | Form submission in progress |
...
- **Form validation errors**: show inline, below the field, after blur — use `TelerikValidationMessage`
- **API errors**: use `TelerikNotification` (bottom-right) for transient errors; inline error banners for blocking errors
...
### Telerik Blazor and mobile

Telerik Blazor is primarily designed for desktop and tablet. For mobile:

- The TelerikGrid does not work well at < 480px. Consider using a `TelerikListView` with card templates for mobile.
- TelerikScheduler is unusable on small screens — provide a fallback list view
- TelerikTreeView is difficult on touch — prefer cascading TelerikDropDownList components on mobile
- TelerikDialog and TelerikWindow should be full-screen on mobile

### Breakpoints

| Name | Width | Typical Telerik Blazor consideration |
|---|---|---|
