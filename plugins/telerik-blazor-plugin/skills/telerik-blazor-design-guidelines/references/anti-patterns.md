...
**Why it's wrong:** Many Telerik Blazor components have pixel-precise column widths and popup
...
**Correct approach:** After changing the font family, test the TelerikGrid with long headers,
TelerikDatePicker calendar at all viewport widths, TelerikDropDownList with long option text, and
TelerikChart axis labels.
...
**Correct approach:** Set explicit `Width` on every `GridColumn`.
...
**Correct approach:** Use pagination (`Pageable="true"`) or row virtualization (`ScrollMode="ScrollMode.Virtual"`).
...
### ❌ Interactive elements in Grid cells without proper keyboard handling
...
**Correct approach:** Use proper `<button>` elements inside `<GridColumn>` `Template` and
...
**Correct approach:** Use the GridColumn's `DisplayFormat` parameter to apply a human-readable date format.
...
**Correct approach:** Use `<button>` or `<TelerikButton>` for actions inside cell templates.
...
### ❌ Using TelerikSplitter for page layout

**Why it's wrong:** Adds unnecessary DOM complexity and JS weight to a problem that CSS
Grid solves statically with zero JS.

**Correct approach:** Use CSS Grid or Flexbox for page structure. Reserve TelerikSplitter for
...
```razor
@* WRONG — creates a new object reference on every render cycle *@
<ChartSeriesItem Data="@(new List<object> { new { x = 1, y = 2 } })">
```
...
**Correct approach:** Define all Chart configuration objects as class properties or fields.

---

### ❌ Inline Grid editing without debouncing change events

**Why it's wrong:** `OnUpdate` events can fire rapidly on every
...
### ❌ Placing multiple Telerik components without a layout container

**Why it's wrong:** Telerik components are rendered as block elements. They stack vertically with
100% width regardless of intent. A Grid will take full width and push Charts below it.

**Correct approach:** Wrap Telerik components in a CSS Grid or Flexbox layout container.
...
`background-color` on `<html>` or `<body>`. The result is dark Telerik components on a
white page.
...
**Correct approach:** Add `min-width: 0` to any flex child that contains a TelerikGrid.

---

### ❌ TelerikGrid without explicit height

**Why it's wrong:** Grid expands indefinitely to show all rows. At 1,000+ rows this
freezes the browser.

**Correct approach:** Always set an explicit `Height` on the TelerikGrid, or use
`flex: 1; min-height: 0` inside a flex column container.

---

### ❌ TelerikChart without explicit height

**Why it's wrong:** Chart renders at 0px height and is invisible.

**Correct approach:** Always set an explicit height and `width: 100%` on the Chart container.

---

### ❌ TelerikDropDownList / TelerikDatePicker left at default width inside forms

**Why it's wrong:** Inline Telerik inputs have an internal minimum width. They will not
stretch to fill their container unless told to.

**Correct approach:** Set `Width="100%"` on the component, or use the Telerik Form
infrastructure (`TelerikForm` / `FormItem`) which handles width automatically.
