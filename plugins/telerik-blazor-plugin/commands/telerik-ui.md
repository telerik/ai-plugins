...
- Project structure (directories, entry points, routing, hosting model)
- Existing Telerik Blazor components already in use and their patterns
- State management approach (services, cascading parameters, etc.)
- Styling approach (CSS Isolation, plain CSS, SCSS, Kendo theme variables)
- Existing test files, test framework (bUnit, xUnit, NUnit), and test patterns
...
2. **Decompose into tasks** — Break the requirement into discrete, ordered tasks. Each task targets a single component or concern. Identify dependencies between tasks. For each task, tag its variant (Build / Extend / Style). Example:
   ```
   Task 1: [Build] TelerikGrid with filtering and sorting (depends on: none)
...
- The Telerik Blazor component names needed for this task
- What aspects to look up: parameters, events, types, accessibility, styling, icons — based on the task variant
...
> - The task involves only project-level wiring (e.g., connecting two already-built components via state) with no new Telerik Blazor APIs → skip
>
...
**Style tasks:** Delegate to the **tb-custom-stylist** subagent with:
- The styling requirement
- The CSS variable and component DOM structure context from Gate 1
- The component files to style

> **Never delegate to tb-developer for Style tasks** — tb-custom-stylist owns all visual-only changes (CSS, layout, spacing, theme overrides).
> **Never delegate to tb-custom-stylist for Build/Extend tasks** — unless the task explicitly requires pixel-perfect CSS beyond what tb-developer handles.
...
- Verification criteria:
  1. The expected Telerik Blazor components are present in the DOM with correct structure
...
If tb-tester reports visual issues (misalignment, broken layout, missing theme, incorrect rendering):
- Re-delegate to the implementing subagent with the screenshots and DOM snapshot as evidence, plus the Gate 1 context
- Re-delegate to tb-tester in browser verification mode after fixes
- Repeat up to **2 iterations**. Log remaining visual issues for the report.

> **When to skip:**
> - The task produced no renderable changes (e.g., only C# services, data models, helper classes, configuration files) → skip
> - The task modified only test files → skip
>
> **Always required when:**
> - Any component Razor, CSS, or layout was created or changed
> - A Style task was completed (the entire point is visual output)

### Gate 4 — Test

Delegate to the **tb-tester** subagent in **test** mode (unit tests, accessibility, validation, visual verification). Provide:
- All files created or modified by the implementing subagent in Gate 2
- The Telerik Blazor API context from Gate 1 (the tester needs API knowledge for correct assertions)
- Test scope: unit tests and accessibility tests at minimum; add validation and visual verification for user-facing components
...
> - **Style-only tasks** → visual verification is sufficient; skip unit tests unless CSS logic was added. Always run accessibility tests.
> - **Data/logic-only tasks** (no UI changes) → unit tests are sufficient; skip visual verification
>
...
If tb-tester reports test failures or accessibility violations caused by the implementation:
1. Re-delegate to the implementing subagent (tb-developer or tb-custom-stylist) with the specific failures and the Gate 1 context
2. Re-delegate to **tb-tester in browser verification mode** if the fix touched any visual code
3. Re-delegate to tb-tester in test mode to verify fixes
4. Repeat up to **3 iterations**. If issues persist, log them and proceed.

If tb-tester reports application code defects (not test issues), collect them for the final report — tb-tester does not modify application source code.
...
After the LAST task passes all applicable gates, delegate to the **tb-reviewer** subagent with:
- All files created or modified across all tasks
- The aggregated Telerik Blazor context from all Gate 1 delegations
- Review scope: component correctness, parameter usage, accessibility, performance, library compliance, infrastructure (TelerikRootComponent, services, imports)

If tb-reviewer finds Critical issues, delegate to **tb-developer** with the issues and context to fix them, then re-delegate to tb-reviewer.
...
> - The change was trivially small (e.g., one parameter change, one label fix) and Gate 4 tests passed → skip
>
> **Always required when:**
...
**Validation**: [N files clean / N issues]
