---
name: telerik-modernize
description: Modernize an existing Blazor application — upgrade legacy patterns, migrate from outdated or third-party UI libraries to Telerik UI for Blazor, audit and fix design system conformance and WCAG accessibility compliance. Works in waves so each wave is independently shippable. Use as the entry point for any application modernization, legacy upgrade, or library migration task.
...
- **NEVER** write Razor markup, C# code, CSS rules, or test assertions — not even for "trivial" modernization changes.
...
**Scoped exception — Wave 0 and Cleanup Wave only:** Package installation (Wave 0) and package removal (Cleanup) are orchestration-level operations. You may execute `dotnet add package` / `dotnet remove package` commands directly for these waves. This exception does NOT extend to writing component code, CSS, or tests in any wave.
...
| Phase 2 | **Design Debt Report** | `tb-designer` subagent |
...
| Phase 5 | **Context Retrieval Report** (per wave) | `tb-context-retriever` subagent |
| Phase 6 | **Pre-Change Design Review** | `tb-designer` subagent |
| Phase 7 | **Developer Report** (per wave) | `tb-developer` subagent |
| Phase 7b | **Styling Report** (per wave) | `tb-stylist` subagent |
| Phase 8 | **Post-Change Design Review** | `tb-designer` subagent |
| Phase 9 | **Test Report** (per wave) | `tb-tester` subagent |
| Phase 10 | Fix confirmation (re-run of Phase 9) | `tb-tester` subagent |
...
- Check if the current directory has a `.csproj` with Blazor references
- If yes: "I found a Blazor project here. What would you like to modernize?"
...
Scan the project to establish: .NET version, Blazor hosting model (Server / WebAssembly / Web App), build tooling, all UI library dependencies (non-Telerik), Telerik NuGet packages and versions, component inventory per file (usage frequency, complexity: Simple/Moderate/Complex), code patterns (code-behind vs inline, dependency injection patterns, state management), styling approach (theming, hardcoded values, design tokens, CSS isolation), accessibility state (keyboard nav, ARIA, labels), test coverage and framework (bUnit/xUnit/NUnit), TypeScript/JavaScript interop usage.

---

## Phase 2: Design Debt Assessment

Delegate to **tb-designer** in **post-implementation review mode** for an initial design audit. Provide a sample of the 5 most-used style files. Request: token usage vs hardcoded values, spacing/typography conformance, WCAG 2.1 AA accessibility gaps, iconography issues.
...
Standard structure:
- **Wave 0 — Foundation**: Install/upgrade Telerik NuGet packages, import theme, configure licensing, register services (`AddTelerikBlazor()`), add `TelerikRootComponent`, verify build.
...
- **Final Wave — Cleanup**: Remove deprecated/source packages, fix build errors, full compliance check.

Present the wave plan as a table with component mappings. Wait for confirmation.

**Skip Wave 0 if** Telerik UI for Blazor is already installed, themed, and licensed.
...
Delegate to **tb-context-retriever** with all Telerik Blazor component names for this wave, aspects (parameters, events, types, accessibility, controlled patterns, migration notes), and for Wave 0 also setup guidance.
...
**First time a wave category appears only** — before modifying files in a new area, delegate to **tb-designer** in post-implementation review mode for the files this wave will touch. Provide the files and the design debt report from Phase 2. This establishes the before-state.
...
**Library migration or code pattern waves:** Delegate to **tb-developer** with the wave description, API context from Phase 5, source files, component mappings, and instruction to preserve functional parity. The developer handles component logic and structure only — **not** final styling.

**Accessibility waves:** Delegate to **tb-developer** with the WCAG findings from Phase 6 or Phase 2 as the fix specification.
...
**Always run after Phase 7** for any wave that produces or modifies renderable Telerik Blazor components. Telerik Blazor components have complex internal DOM structures that require specialized styling — all theming and CSS customization must go through the stylist.
...
**Skip for** Wave 0 and Cleanup wave. **Skip ONLY if** the wave produced exclusively non-renderable artifacts (C# interfaces only, configuration only, data services with no Razor markup). If the wave produced ANY Razor component with markup output, Phase 7b is mandatory.

---

## Phase 8: Post-Change Design Review

Delegate to **tb-designer** in post-implementation review mode with files from Phases 7 and 7b, the before-state from Phase 6, and design token context. The agent verifies conformance improved or was maintained, token usage is correct, and WCAG 2.1 AA is met.
...
**Browser verification** — Delegate to **tb-tester** in browser verification mode with files from Phases 7 and 7b, API context, pages/routes, and criteria (visual match, functional parity, correct styling, no console errors). If regressions:
- **Visual/CSS regressions** → re-delegate to **tb-stylist**
- **Structural/logic regressions** → re-delegate to **tb-developer**
Re-verify (up to 2 iterations).

**Skip browser verification for** Wave 0 and Cleanup.

**Testing** — Build check is always required. Then delegate to **tb-tester** in test mode. **Testing is MANDATORY for every wave that produces or modifies code — no exceptions.** The absence of existing test files is NOT permission to skip — it is the trigger to create new tests. Scope depends on wave type: migration waves (update tests to Telerik Blazor selectors), pattern waves (update patterns), design token waves (visual regression + accessibility), accessibility waves (validate every fix).
...
1. **Visual/CSS failures** → re-delegate to **tb-stylist** with failures, screenshots, and design debt context.
...
After all waves complete, delegate to **tb-designer** with all files across all waves, the Phase 2 design debt report as baseline, and request a final cross-cutting conformance check.
...
Delegate to **tb-reviewer** with all files across all waves, aggregated API context, and review scope (correctness, C# types, parameter usage, accessibility, performance, library compliance, security).
...
| Metric | Before | After |
|--------|--------|-------|
| UI library imports | [source library] | Telerik UI for Blazor only |
...
### Validation
- Build: [PASS/FAIL]
- Browser verification: [PASS / regressions noted — sourced from tb-tester]
...
- Design compliance: [PASS / issues resolved — sourced from tb-designer's reports]
