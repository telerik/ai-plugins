---
name: kendo-migrator
description: Use this agent when the user wants to migrate an entire project (or a significant part of one) from any UI component library to KendoReact. This agent conducts a thorough discovery interview, analyzes the source project, creates a detailed migration plan, executes the migration wave by wave, and validates each wave before proceeding. Trigger when the user mentions migrating from MUI, Ant Design, Chakra, Bootstrap, Angular Material, or any other UI library to KendoReact, or wants to convert an existing application to use KendoReact exclusively. Examples:

<example>
Context: User has a React app built with MUI and wants to switch to KendoReact.
user: "Migrate my app from MUI to KendoReact"
assistant: "I'll use the kendo-migrator agent to analyze your project, plan the migration, and execute it step by step."
<commentary>
Full project migration from a named UI library to KendoReact is the primary trigger for kendo-migrator.
</commentary>
</example>

<example>
Context: User has a mixed project with Ant Design and some custom components.
user: "I want to replace all Ant Design components with KendoReact equivalents"
assistant: "Let me use kendo-migrator to inventory your Ant Design usage, map each component to KendoReact, and migrate them systematically."
<commentary>
Library replacement request triggers kendo-migrator for systematic migration.
</commentary>
</example>

<example>
Context: User wants to convert a Chakra UI app but keep existing business logic.
user: "Convert my Chakra UI app to use KendoReact — keep the same functionality"
assistant: "I'll use kendo-migrator to carefully analyze your app, preserve all business logic, and replace only the UI layer with KendoReact."
<commentary>
Conversion with explicit business logic preservation is a strong kendo-migrator trigger.
</commentary>
</example>

<example>
Context: User has a large app with multiple UI libraries and wants consolidation.
user: "My app uses MUI for the grid, react-select for dropdowns, and react-datepicker for dates. Consolidate everything to KendoReact."
assistant: "I'll use kendo-migrator to map all three libraries to their KendoReact equivalents and migrate them in a coordinated plan."
<commentary>
Multi-library consolidation to KendoReact triggers kendo-migrator.
</commentary>
</example>

<example>
Context: User wants to migrate a non-React app (Angular, Vue) to React with KendoReact.
user: "Migrate my Angular Material app to React with KendoReact"
assistant: "I'll use kendo-migrator to analyze the Angular project, map all components and patterns to React + KendoReact, and execute the migration."
<commentary>
Cross-framework migration to React + KendoReact triggers kendo-migrator.
</commentary>
</example>

<example>
Context: User opens a React project and the assistant detects non-KendoReact UI library imports (MUI, Ant Design, Chakra, etc.) alongside KendoReact.
user: "I want to standardize on one UI library for this project"
assistant: "I see your project uses both MUI and KendoReact. I'll use the kendo-migrator agent to plan and execute the consolidation to KendoReact."
<commentary>
When mixed UI libraries are detected and the user expresses intent to standardize, proactively trigger kendo-migrator to consolidate everything to KendoReact.
</commentary>
</example>

<example>
Context: User mentions they inherited a codebase with an older or different UI library.
user: "I inherited this project and it uses Ant Design — we're a KendoReact shop now"
assistant: "I'll use the kendo-migrator agent to inventory the Ant Design usage and create a migration plan to KendoReact."
<commentary>
Implicit migration need — user indicates the current UI library doesn't match their standard. Proactively trigger kendo-migrator.
</commentary>
</example>
