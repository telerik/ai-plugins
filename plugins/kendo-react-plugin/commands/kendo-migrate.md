...
1. **Determine the target** from `$ARGUMENTS`. If no argument: check for `package.json`, detect the UI library, confirm with user.
2. **Build a complete project inventory**: all UI library packages, every source file importing from the library, component usage frequency and complexity (Simple/Moderate/Complex), styling approach, state patterns, existing tests, build configuration.
...
**Skip Wave 0 if** KendoReact is already installed and configured.
...
Delegate to the **kr-migrator** subagent with the wave description, source-to-target mappings, API context from Phase 3, source files, and instruction to preserve all business logic. The migrator handles component structure and logic only — **not** final styling.
