# CI, Containers & Build Environments — KendoReact Licensing

Branches for environment-specific activation: CI/CD pipelines, Docker, pnpm/Yarn workspaces, SSR/Next.js builds, test runners (Jest/Vitest/Storybook), registry/proxy access, and CI secret-size limits. Read the branch matching the build environment.
...
### SSR and Next.js Build Context

Use when the watermark appears in a Next.js, Remix, or other SSR framework production build but not during local development.

```yaml
strong_signals:
    - Watermark does not appear in 'next dev' but appears after 'next build' or in production deployment.
    - Activation was run before 'next dev' but not before 'next build'.
...
key_behaviors:
    ...
    - 'npx kendo-ui-license activate' must be run before 'next build', not only before 'next dev'.
    - '.env.local' is not committed and is not available in CI unless the variable is also set as a CI secret.

recommended_resolution:
    - Add 'npx kendo-ui-license activate' as a distinct CI step before the 'next build' step.
...
avoid:
    - Assuming 'next dev' activation covers 'next build' in the same session.
...
```text
For Next.js and SSR projects, license activation must occur before each production build ('next build'), not only before the dev server ('next dev'). Please add 'npx kendo-ui-license activate' as a dedicated step before your build step in CI, and ensure TELERIK_LICENSE is injected as an environment variable — do not use the NEXT_PUBLIC_ prefix, as that would embed the key in the browser bundle.
```
