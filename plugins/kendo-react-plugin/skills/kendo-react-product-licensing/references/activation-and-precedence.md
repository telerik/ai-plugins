# Activation & Precedence Troubleshooting — KendoReact Licensing
...
```yaml
strong_signals:
    - Different Kendo package major versions in same app (for example KendoReact v5 and v9 side by side).
...
### Vite Build Cache Stale After Re-activation
...
recommended_resolution:
    - Delete the Vite build cache: rm -rf node_modules/.vite
    - Re-run: npx kendo-ui-license activate
    - Restart the Vite dev server or re-run the build.

avoid:
    - Repeating refresh/activate cycles without clearing the Vite cache first.
    - Treating Vite cache persistence as a key validity problem when activation command itself reports success.
...
```text
Vite caches compiled output and does not automatically invalidate that cache when a license key changes. Please delete the node_modules/.vite folder, then re-run 'npx kendo-ui-license activate' and start a fresh Vite dev server. This resolves most cases where activation succeeds but the watermark still appears.
```
