# Account, Entitlement & Legal — KendoReact Licensing
...
```yaml
strong_signals:
    - Customer has valid KendoReact license but warning appears from Kendo UI for jQuery or Kendo UI for Angular package.
...
```text
This component became available without a license requirement in a specific major version of KendoReact. If you are on an older major release, a license is still required for that version line. The recommended path is upgrading to the latest supported major where the component is free. If staying on the older major is required, please contact Sales for licensing options.
```
...
strong_signals:
    ...
    - Current behavior shows popup/checks for any developer who runs or builds a project containing premium KendoReact features.
...
```text
Thank you for sharing the earlier guidance. Based on current licensing enforcement, any developer who runs or builds a project containing premium KendoReact features must have valid license entitlement.
