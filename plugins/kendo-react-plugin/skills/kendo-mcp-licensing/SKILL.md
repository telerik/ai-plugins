...
This skill diagnoses and resolves **MCP server entitlement failures** for `@progress/kendo-react-mcp`. It is the right skill when MCP tool calls return `PERMISSION_DENIED` despite the server starting correctly, when premium MCP tools don't appear in the IDE, or when the developer asks whether their license covers MCP AI tools.

This skill does NOT cover:

- Watermarks, banners, or TKL activation codes in the app — route to `kendo-react-product-licensing`
- First-time install, onboarding, or "how do I get started" — route to `kendo-react-getting-started`
...
path_notes:
    ...
    ai_coding_assistant_deprecated_migration: >
        ...
        (#kendo_react_assistant) and needs to migrate to the Agentic UI
...
entitlement_rules:
    ...
    - As of @progress/kendo-react-mcp v1.4, the server automatically recognizes the Telerik license and activates the appropriate tools.
...
triage_order:
    ...
    4_confirm_mcp_server_version: >
        Run: npm view @progress/kendo-react-mcp version
        Auto-recognition requires v1.4+. If older, update: npx -y @progress/kendo-react-mcp@latest
...
Configured automatically by `npx kendo setup react`. Manual setup: follow JetBrains MCP documentation and point to the same npx command.
...
ide_config_pitfalls:
    ...
    server_version_too_old:
        description: Running @progress/kendo-react-mcp < v1.4 which lacks auto-recognition.
...
The AI Coding Assistant (`#kendo_react_assistant`) was deprecated in February 2026 and replaced by the Agentic UI Generator (`#kendo_ui_generator`).

```yaml
migration:
    old_tool_handle: '#kendo_react_assistant'
    ...
    what_to_do:
        - Update prompts from '#kendo_react_assistant' to '#kendo_ui_generator'.
        - Update @progress/kendo-react-mcp to @latest.
...
sales_handoff:
    ...
    backend_sync_suspected:
        ...
        payload:
            - Account email
            - @progress/kendo-react-mcp version
...
To evaluate the AI tools, start a free 30-day trial: https://www.telerik.com/try/kendo-react-ui
...
```text
The AI Coding Assistant (#kendo_react_assistant) was deprecated in February 2026 and
replaced by the Agentic UI Generator (#kendo_ui_generator).

To migrate:
  1. Update the MCP server: ensure @progress/kendo-react-mcp@latest is in your mcp.json
  2. Use #kendo_ui_generator instead of #kendo_react_assistant in your prompts
...
a free 30-day trial: https://www.telerik.com/try/kendo-react-ui
```
...
route_to_kendo_react_getting_started_when:
    ...
    - Developer asks how to install KendoReact or which tier to choose
...
avoid:
    ...
    - Referencing the deprecated #kendo_react_assistant handle without explaining the migration to #kendo_ui_generator.
...
note: >
    ...
    licensing issue first (kendo-react-product-licensing), then re-check MCP
