# Advanced Diagnostics — KendoReact MCP Licensing
...
```text
 1. Which MCP package and version is used (e.g. @progress/kendo-react-mcp@x.y.z)?
...
```yaml
strong_signals:
    - Customer installs '@progress/kendo-react-mcp' but account license is for Kendo UI for Angular or jQuery.
...
```text
The MCP package you installed may require entitlement for a different product family than your activated license covers. Please confirm that the MCP package name matches your licensed product (for example @progress/kendo-react-mcp requires a Kendo UI for React license) and that activation was run in the workspace for that product.
```
...
```json
{
    ...
    "product": "Kendo UI for React",
    "feature": "Kendo React MCP Server / AI Tools",
    "customer_environment": {
        "product": "Kendo UI for React",
        "mcp_package": "@progress/kendo-react-mcp",
